...
void setup() {
  pinMode(joystickButonPin, INPUT_PULLUP);
...
void loop() {
  ...
  int jX = analogRead(joystickXPin);
  int jY = analogRead(joystickXPin);
  int buton = digitalRead(joystickButonPin);
  ...
  //JOYSTICK Y EKSENINDE HAREKET EDERSE
  if (jY > 2100 || jY < 1600) {
